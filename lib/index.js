...
Router.prototype.getMethods = function (route) {
    ...
    result = result.map(function (method) {
...
Router.prototype.getController = function (route) {
    ...
    var controllerFile = fs.statSync(controllerPath, function (error, stat) {
        if (error !== null) {
...
Router.prototype.getFormat = function (route) {
    ...
    if (route.defaults.hasOwnProperty(_format) && this.FORMATS.hasOwnProperty(route.defaults._format)) {
        result = route.defaults._format;
    } else if (typeof this.getTemplate(route) == 'undefined') {
        result = 'html';
    } else {
        result = 'json';
    }
